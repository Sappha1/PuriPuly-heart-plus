...
_BUILD_TAG = "r391-steam-beta"  #increment each build so user can confirm version
...
class DashboardView(ft.Row):
    ...
    def _on_auto_detect_badge_click(self, _e=None) -> None:
    ...
    def _sync_translation_button_state(self) -> None:
        ...
        for attr in ("_lang_panel", "_peer_panel"):
            ...
            panel.tooltip = (
                ...
                else t("dashboard.tooltip.lang_panel_trans_off")
            )
            try:
                panel.update()
            except Exception:
