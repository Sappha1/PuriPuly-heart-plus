...
class SettingsView(ft.Column):
    ...
    def _build_ui(self) -> None:
        ...
        self._ptt_mute_sync_text = self._build_clickable_text(
            t("settings.option.off"),
            self._on_ptt_mute_sync_click,
        )
        ptt_mute_sync_card = self._wrap_unit_card(
            title=self._info_title_keyed(
                "settings.ptt_mute_sync", "settings.ptt_mute_sync.tooltip"),
            value=self._ptt_mute_sync_text,
        )
        ...
        self._speaker_id_title = ft.Text(
            t("settings.speaker_id"),
            size=13,
            weight=ft.FontWeight.BOLD,
            color=COLOR_NEUTRAL,
        )
        self._speaker_id_card = self._wrap_unit_card(
            title=self._info_title(self._speaker_id_title,
                t("settings.speaker_id.tooltip")),
    ...
    def load_from_settings(
        ...
        *,
        ...
        self._ptt_mute_sync_text.content.value = t(
            "settings.option.on" if bool(getattr(settings.ui, "ptt_mute_sync", False)) else "settings.option.off"
        )
    ...
    def _on_ptt_mute_sync_click(self, e) -> None:
        if not self._settings:
            return
        new_value = not bool(getattr(self._settings.ui, "ptt_mute_sync", False))
        self._settings.ui.ptt_mute_sync = new_value
        self._ptt_mute_sync_text.content.value = t(
            "settings.option.on" if new_value else "settings.option.off"
        )
        if self.page:
            self._ptt_mute_sync_text.update()
        self._emit_settings_changed()
    ...
    def _on_speaker_id_click(self, e) -> None:
        ...
        current = "on" if getattr(self._settings.stt, "speaker_id", True) else "off"
        modal = SettingsModal(
            self.page, t("settings.speaker_id"), options,
            self._on_speaker_id_selected, show_description=False,
        )
        modal.open(current)

    def _on_speaker_id_selected(self, value: str) -> None:
        if not self._settings:
            return
        self._settings.stt.speaker_id = value == "on"
        self._speaker_id_text.content.value = t(
            "common.on" if self._settings.stt.speaker_id else "common.off"
        )
        if self.page:
            self._speaker_id_text.update()
        self._emit_settings_changed()
